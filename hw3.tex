...
% Question 1
\question 
Use the Principle of Mathematical Induction to show this inequality is true for all  integers $n\geq 2$: \\$\displaystyle\sum_{i=1}^n\df{1}{\sqrt{i\,\,}} > \sqrt{n}$ \\
\\
\solution
\begin{proof}
    By induction. \\
    ...
    \begin{align*}
        ...
        \sum_{i=1}^{k}{\frac{1}{\sqrt{i}} + \frac{1}{\sqrt{k+1}}} &> \sqrt{k} + \frac{1}{\sqrt{k+1}} \\
        \sum_{i=1}^{k+1}{\frac{1}{\sqrt{i}}} &> \sqrt{k} + \frac{1}{\sqrt{k+1}}
    \end{align*}
    Using the Inductive Hypothesis, we know the above inequality is true. Thus, it is sufficient to prove that $\sqrt{k} + \df{1}{\sqrt{k+1}} > \sqrt{k+1}$ by the transitive property.
    \begin{align*}
        \sqrt{k} + \frac{1}{\sqrt{k+1}}                   &\stackrel{?}{>} \sqrt{k+1} \\
        \sqrt{k+1}\paren*{\sqrt{k} +\frac{1}{\sqrt{k+1}}} &\stackrel{?}{>} \sqrt{k+1}\sqrt{k+1} \\
        \sqrt{k^2 + k} + 1                                &\stackrel{?}{>} k + 1 \\
        \sqrt{k^2 + k}                                    &\stackrel{?}{>} k \\
        k^2 + k                                           &\stackrel{?}{>} k^2 \\
        k                                                 &>               0
    \end{align*}
    Since $k \geq 2$, $k > 0$ is true. Thus, the inequality $\sqrt{k} + \df{1}{\sqrt{k+1}} > \sqrt{k+1}$ is true. Therefore, $P(k+1)$ is true using the Inductive Hypothesis; $P(k) \implies P(k+1)$. \\
    \\
